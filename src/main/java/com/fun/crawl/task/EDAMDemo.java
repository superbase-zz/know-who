package com.fun.crawl.task;/*
  ...
  To compile (Unix):
    javac -classpath ../../target/evernote-api-*.jar com.fun.crawl.task.EDAMDemo.java
 
  To run:
    java -classpath ../../target/evernote-api-*.jar com.fun.crawl.task.EDAMDemo
...
//            demo.updateNoteTag();
    ...
    public EDAMDemo(String token) throws Exception {
        ...
        boolean versionOk = userStore.checkVersion("Evernote com.fun.crawl.task.EDAMDemo (Java)",
    ...
    public void listNotes() throws Exception {
        ...
        for (Notebook notebook : notebooks) {
            ...
            System.out.println("Notebook: " + notebook.getGuid());
            if (notebook.getName().indexOf("得到专栏-Dr魏的家庭教育宝典-完结")!=-1){
                tonoteBookgUid=notebook.getGuid();
            }
...
//
//
//                Note copy = note.deepCopy();
//                List<Resource> resources1 = copy.getResources();
//                for (Resource resource : resources1) {
//                    System.out.println(resource);
//                }
//
//                copy.setTitle("1adadad"+note.getTitle());
//                    noteStore.copyNote(note.getGuid(),tonoteBookgUid);
//
...
//                noteStore.copyNote();
        ...
        for (LinkedNotebook linkedNotebook : linkedNotebooks) {
            ...
            if (linkedNotebook.getShareName().equals("百部世界文学经典课")) {
                System.out.println("GUID: " + linkedNotebook.getGuid());
                System.out.println("noteStoreUrl: " + linkedNotebook.getNoteStoreUrl());
                String shareKey = linkedNotebook.getShareKey();
                System.out.println(shareKey);

                THttpClient tHttpClient = new THttpClient(linkedNotebook.getNoteStoreUrl());
...
//                NoteStoreClient
                ...
                AuthenticationResult authenticationResult = client.authenticateToSharedNotebook(shareKey, AUTH_TOKEN);
...
//                System.out.println(noteaa.toString());
...
//            NoteStoreClient client = linkedNoteStoreClient.getClient();
//)
//            NoteFilter noteFilter =new NoteFilter();
//            noteFilter.setNotebookGuid(linkedNotebook.getGuid());
//            NoteList notes = client.findNotes(noteFilter, 0, 100);
//            List<Note> noteList = notes.getNotes();
//            for (Note note : noteList) {
//                System.out.println(note.getTitle());
//
...
//            }
    ...
    private void createNote() throws Exception {
        ...
        Note note = new Note();
        note.setTitle("Test note from com.fun.crawl.task.EDAMDemo.java");
    ...
    private void searchNotes() throws Exception {
        ...
        // In this example, we search for notes that have the term "com.fun.crawl.task.EDAMDemo" in
        // the title.
        // This should return the sample note that we created in this demo app.
        String query = "intitle:com.fun.crawl.task.EDAMDemo";
